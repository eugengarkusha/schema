...
class CSVParserTest extends FlatSpec with Matchers {
  ...
  val row = Record(a = 12, b = "blah", c = Record(d = "foo", e = true), f = "bar")
  ...
  "default CSV parser" should "be deriveable from the abstract schema " in {
    ...
    p1.parse("12,blah,foo,true,bar") should be (Success(row))
  }
  ...
  "derived default field parsers" should "be customizable " in {
    ...
    p2.parse("12,blah,foo,true,bar") should be(Success(row.updateWith('a)(_ + 2)
                                                          .updateWith('b)(_ + "!!!")))
  }
  ...
  "derived default field parsers" should "be mappable to arbitraty index in CSV string" in {
    ...
    p3.parse("blah,foo,bar,12,true") should be (Success(row))
  }
  ...
  "derived default field parsers" should "be customizable and indexable simultaneously" in {
    ...
    p4.parse("blah,foo,bar,12,true") should be(Success(row.updateWith('a)(v=> if (v > 11) 100 else 0)
                                                          .updateWith('b)(_ + "111")
